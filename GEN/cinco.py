...
# Função para garantir que pais diferentes sejam escolhidos
def select_different_parents(population, fitness_values):
    parent1 = tournament_selection(population, fitness_values)
    parent2 = tournament_selection(population, fitness_values)
    # Garantir que parent1 e parent2 são diferentes
...
for generation in range(generations):
    ...
    for _ in range(population_size // 2):
        parent1, parent2 = select_different_parents(population, fitness_values)
...
best_individual = max(population, key=lambda ind: fitness(ind))
best_fitness = fitness(best_individual)
# print(f"Melhor Indivíduo Final: {best_individual}, Aptidão = {best_fitness}, Valor x = {decode_individual(best_individual)}")
